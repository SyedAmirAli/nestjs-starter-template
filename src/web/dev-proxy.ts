...
import { isAdminConsolePath } from './console-path';
import { isReservedApiPath } from './reserved-paths';

/**
 * Development: reverse-proxy `/admin` to the Vite dev server.
 *
...
/**
 * Attaches to the http server's `upgrade` event so Vite's HMR websocket survives.
 *
 * Login is a public SPA route, so HMR cannot require an admin session — the sign-in screen
 * would otherwise be a static page. Upgrades that are not under `/admin` are left alone.
 */
...
export function createDevProxy(): DevProxy {
    const proxy = createProxyMiddleware({
        ...
        // `upgrade` event itself on the first proxied request, which would swallow upgrades
        // for the whole process. attachUpgrade() below does the subscription, scoped to
        // `/admin`.
        ws: false,
    ...
    return {
        ...
        attachUpgrade: (server) => {
            server.on('upgrade', (req: IncomingMessage, socket: Socket, head: Buffer) => {
                ...
                if (!isAdminConsolePath(pathOf(req))) return;
...
<body><main>
  ...
  <p>The API is up and proxying <code>/admin</code> to the console&rsquo;s dev server, but nothing is listening there.</p>
